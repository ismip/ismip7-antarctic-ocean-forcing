...
class Region:
    ...
    def plot_delta(self):
        fig,ax = plt.subplots(1,5,figsize=(5*3.5,4.5),sharex=True,sharey=True)
        ...
        im = ax[3].pcolormesh(self.prd.Sb,self.prd.Tb,self.fut.deltaTf.T,cmap='cmo.balance',vmin=-3,vmax=3)

        plt.colorbar(im,ax=ax[3],orientation='horizontal')
        ax[3].set_title(f'Filled')
        #ax[1].plot(self.fut.S[:,jj,ii],self.fut.T[:,jj,ii],c='tab:red')

        im = ax[4].pcolormesh(self.ref.Sb,self.ref.Tb,self.fut.dTcorb.T,cmap='cmo.balance',vmin=-3,vmax=3)
    ...
    def plot_profiles(self,vlim=-1):
        ...
        ax[1].plot(Tav,self.ref.depth[:vlim],label='optimised',c='tab:red',ls='--')
        ...
        Tav = np.average(np.where(np.isnan(self.ref.T[:vlim,:,:]),0,self.fut.dTcor[:vlim,:,:]),axis=(1,2),weights=self.ref.V[:vlim,:,:])
        ax[2].plot(Tav,self.ref.depth[:vlim],c='tab:red')

        Tav = np.average(np.where(np.isnan(self.prd.T[:vlim,:,:]),0,self.fut.dTraw[:vlim,:,:]),axis=(1,2),weights=self.prd.V[:vlim,:,:])
        ax[2].plot(Tav,self.prd.depth[:vlim],c='.5')

        Tav = np.average(np.where(np.isnan(self.ref.T[:vlim,:,:]),0,(self.ref.T+self.fut.dTcor)[:vlim,:,:]),axis=(1,2),weights=self.ref.V[:vlim,:,:])
    ...
    def get_bias(self,Tperc=.99,Sperc=.99):
        ...
        for A,AA in enumerate(a):
            ...
            #rmse[A] = np.sum(np.where(prd==0,0,(np.log10(prd/np.sum(prd))-np.log10(ref/np.sum(ref))))**2) / np.sum(np.where(prd==0,0,1))**.5
            ...
            for j in range(self.fut.dTcor.shape[1]):
                for k in range(self.fut.dTcor.shape[2]):
                    if self.ref.V[i,j,k]==0:
                        ...
                        self.fut.dScor[i,j,k] = self.fut.deltaSf[isref,jtref]
                        self.fut.dTcor[i,j,k] = self.fut.deltaTf[isref,jtref]

        #Distributions of dS and dT, only for plotting purposes
        VTdel,Sdel,Tdel = np.histogram2d(self.ref.S.flatten()[self.ref.V.flatten()>0],self.ref.T.flatten()[self.ref.V.flatten()>0],bins=100,range=[[self.ref.Sb[0],self.ref.Sb[-1]],[self.ref.Tb[0],self.ref.Tb[-1]]],weights=(self.ref.V*self.fut.dTcor).flatten()[self.ref.V.flatten()>0])
        self.fut.dTcorb = VTdel/self.ref.Vb
        VSdel,Sdel,Tdel = np.histogram2d(self.ref.S.flatten()[self.ref.V.flatten()>0],self.ref.S.flatten()[self.ref.V.flatten()>0],bins=100,range=[[self.ref.Sb[0],self.ref.Sb[-1]],[self.ref.Tb[0],self.ref.Tb[-1]]],weights=(self.ref.V*self.fut.dScor).flatten()[self.ref.V.flatten()>0])
    ...
    def get_profiles(self):
        ...
        self.ref.Tz[:kmax] = np.average(np.where(np.isnan(self.ref.T[:kmax,:,:]),0,self.ref.T[:kmax,:,:]),axis=(1,2),weights=self.ref.V[:kmax,:,:])
        self.ref.Sz[:kmax] = np.average(np.where(np.isnan(self.ref.S[:kmax,:,:]),0,self.ref.S[:kmax,:,:]),axis=(1,2),weights=self.ref.V[:kmax,:,:])

    def calc_horav(self,var,run):
        '''Calculate horizontally averaged vertical profile'''

        try:
            out = np.average(np.where(np.isnan(var),0,var),axis=(1,2),weights=run.V)
        except:
            out = np.nan*np.ones(var.shape[0])
            kmax = np.where(np.sum(run.V,axis=(1,2))==0)[0][0]
            out[:kmax] = np.average(np.where(np.isnan(var[:kmax,:,:]),0,var[:kmax,:,:]),axis=(1,2),weights=run.V[:kmax,:,:])
